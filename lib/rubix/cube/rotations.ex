defmodule Rubix.Cube.Rotations do
  ...
  @rotation_matrix %{
    ...
    "F" => %{
      {:up, 3, 1} => {:right, 1, 1}, {:up, 3, 2} => {:right, 2, 1}, {:up, 3, 3} => {:right, 3, 1},
      {:right, 1, 1} => {:down, 1, 3}, {:right, 2, 1} => {:down, 1, 2}, {:right, 3, 1} => {:down, 1, 1},
      {:down, 1, 1} => {:left, 1, 3}, {:down, 1, 2} => {:left, 2, 3}, {:down, 1, 3} => {:left, 3, 3},
      {:left, 1, 3} => {:up, 3, 3}, {:left, 2, 3} => {:up, 3, 2}, {:left, 3, 3} => {:up, 3, 1}
    },
    "F'" => %{
      {:up, 3, 1} => {:left, 3, 3}, {:up, 3, 2} => {:left, 2, 3}, {:up, 3, 3} => {:left, 1, 3},
      {:right, 1, 1} => {:up, 3, 1}, {:right, 2, 1} => {:up, 3, 2}, {:right, 3, 1} => {:up, 3, 3},
      {:down, 1, 1} => {:right, 3, 1}, {:down, 1, 2} => {:right, 2, 1}, {:down, 1, 3} => {:right, 1, 1},
      {:left, 1, 3} => {:down, 1, 1}, {:left, 2, 3} => {:down, 1, 2}, {:left, 3, 3} => {:down, 1, 3}
    },
    "B" => %{
      {:up, 1, 1} => {:left, 3, 1}, {:up, 1, 2} => {:left, 2, 1}, {:up, 1, 3} => {:left, 1, 1},
      {:right, 1, 3} => {:up, 1, 1}, {:right, 2, 3} => {:up, 1, 2}, {:right, 3, 3} => {:up, 1, 3},
      {:down, 3, 1} => {:right, 3, 3}, {:down, 3, 2} => {:right, 2, 3}, {:down, 3, 3} => {:right, 1, 3},
      {:left, 1, 1} => {:down, 3, 1}, {:left, 2, 1} => {:down, 3, 2}, {:left, 3, 1} => {:down, 3, 3}
    },
    "B'" => %{
      {:up, 1, 1} => {:right, 1, 3}, {:up, 1, 2} => {:right, 2, 3}, {:up, 1, 3} => {:right, 3, 3},
      {:right, 1, 3} => {:down, 3, 3}, {:right, 2, 3} => {:down, 3, 2}, {:right, 3, 3} => {:down, 3, 1},
      {:down, 3, 1} => {:left, 1, 1}, {:down, 3, 2} => {:left, 2, 1}, {:down, 3, 3} => {:left, 3, 1},
      {:left, 1, 1} => {:up, 1, 3}, {:left, 2, 1} => {:up, 1, 2}, {:left, 3, 1} => {:up, 1, 1}
    }
  }
